...
<body>
    ...
    <section id="pricing" class="pricing pt-0">
     ...
     <i class="bi bi-arrow-left"></i>
          <a  href="#" style="font-size: 1.2rem; color: rgba(0,0,0,1); margin-left: .5rem;"> Approve Tutorial</a>
</a>
...
<div class="fetch-room-mate">
    ...
    $sql1 = "SELECT * FROM tutorial WHERE tutorial_id='$id' AND status='pending'";
        $stmt1 = $conn->prepare($sql1);
        $stmt1->execute();
        $result1 = $stmt1->get_result();
        if($result1->num_rows > 0){
            while($row1 = $result1->fetch_assoc()){
            $user_id = $row1['teacher_id'];
    ...
    <div class="col-lg-4" data-aos="fade-up" data-aos-delay="100">
      ...
      <h4 style="font-size: 1.3rem; color: rgba(0,0,0,.6);">#<?= $row6['school'].' #'.$row6['department'].' #'.$row1['course_title'].' #'.$row1['description']; ?></h4>
...
<div class="container position-relative" data-aos="fade-up">
  ...
  <button name="btn" id="btn" class="buy-btn">Approve Tutorial</button>
       </form>
...
<script>
    ...
    $(document).on('click', '#btn', function(e){
        ...
        if(agent_id == ""){
            ...
            $.ajax({
                url: 'backend/approve-tutorial2.php',
                type: 'post',
                data:
                {
                    btn: btn,
