...
if(isset($_POST['quality']) && isset($_POST['user_id']) && isset($_POST['product_id'])){
    ...
    $sql = "UPDATE orders SET quantity=?, product_id=?, user_id=?, order_date=NOW(), payment_status='pending' WHERE order_id =?";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param('ssss', $quality, $product_id, $id, $item_id);
    if($stmt->execute()){
