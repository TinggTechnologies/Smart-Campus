...
if(isset($_GET['tutorial_id'])){
    $item_id = $_GET['tutorial_id'];
}

if(isset($_SESSION['id'])){
    $id = $_SESSION['id'];
}
$_SESSION['tutorial_id'] = $item_id;
...
$sql2 = "SELECT * FROM tutorial WHERE tutorial_id=?";
...
?>
  ...
  if($price === 0){
    ?>
    <a href="../download.php?path=uploads/<?= $row2['container']; ?>" class="btn_3" style="margin-top: 1rem;">Download  Tutorial</a>
    <?php
  } else {
     ?>
 <form id="paymentForm">
...
function payWithPaystack(e) {
  ...
  let handler = PaystackPop.setup({
    ...
    callback: function(response){
      ...
      alert(message);
      window.location = "http://localhost/easylearn/store/verify-tutorial-transaction.php?reference=" + response.reference;
    }
  });
